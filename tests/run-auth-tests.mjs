...
import { getSafeAvatarUrl, isMalformedAvatarValue } from '../src/lib/avatar.ts';
import { inspectCookieHeader } from '../src/lib/auth-diagnostics.ts';

test('1. Auth Contract & Allowlist Rules', async (t) => {
  await t.test('strips forbidden rich profile fields from user_metadata', () => {
    const input = {
      full_name: 'Jane Doe',
      name: 'Jane Doe',
      email: 'jane@example.com',
      bio: 'Rich biography text that belongs in PostgreSQL.',
      ...
      untrustedField: 'malicious_input',
    };
  ...
  await t.test('allows only authorization keys in app_metadata', () => {
    const input = {
      role: 'Admin',
      team: "CEO's Office",
      is_alumni: true,
...
test('2. Avatar Safety & Data URI Rejection', async (t) => {
  await t.test('accepts valid HTTP/HTTPS URLs', () => {
  ...
  await t.test('rejects oversized URLs (> 2048 chars)', () => {
...
test('4. Cookie Header Diagnostics & Bounded State', async (t) => {
  await t.test('inspects cookie header size without leaking token values', () => {
  ...
  await t.test('flags oversized Cookie headers', () => {
...
test('5. Zero-Crash Resilience on Malformed Data', async (t) => {
  await t.test('handles null, undefined, and non-object inputs safely', () => {
    assert.doesNotThrow(() => sanitizeUserAuthMetadata(null));
    assert.doesNotThrow(() => sanitizeUserAuthMetadata(undefined));
    assert.doesNotThrow(() => sanitizeUserAuthMetadata({ full_name: null }));
