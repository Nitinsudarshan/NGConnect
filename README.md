...
* **Role-Based Access Control (RBAC):** Fine-grained permissions powered by database-backed resource permissions (`rbac_permissions`), Supabase RLS, and Next.js Middleware (`src/middleware.ts` for auth session management). Permissions are granted per role and can be overridden for an individual user. Roles include **Super Admin**, **Admin**, **Manager**, **Program**, **Operations**, **Viewer**, and **Member**. Users are also categorized into teams (**CEO's Office**, **Alumni Growth**, **PNC**, **Finance**, **None**), which are organisational labels only and grant no access. See [`docs/ROLES.md`](docs/ROLES.md).
