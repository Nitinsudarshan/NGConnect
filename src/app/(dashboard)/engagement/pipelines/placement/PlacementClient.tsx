...
const STAGES = ["No Response", "Call Back", "Did Not Connect", "Discussed", "Invalid Number"];

export default function PlacementClient({ pipeline, memberships, userEmail }: PlacementClientProps) {
  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <Briefcase className="w-6 h-6 text-indigo-500" /> Placement Support Board
          </h1>
          <p className="text-xs text-muted-foreground mt-1">
            Outreach tracking for placement support, job transitions, and working status updates.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {STAGES.map((stage) => {
          const cards = filtered.filter((m) => m.status === stage || (!STAGES.includes(m.status) && stage === "Call Back"));
