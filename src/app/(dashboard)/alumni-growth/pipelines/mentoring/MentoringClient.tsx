...
import { GraduationCap, ChevronRight, ChevronDown, Filter, ChevronsRightLeft, ChevronsLeftRight } from "lucide-react";
...
function MentoringColumn({ stage, activeStages, filters, userEmail, collapsed, setCollapsed, pocOptions }: { stage: any, activeStages: any[], filters: any, userEmail: string, collapsed: boolean, setCollapsed: (v: boolean) => void, pocOptions?: {email: string; name: string}[] }) {
  ...
  const fetchCards = async (pageNum: number, overwrite: boolean = false) => {
    setLoading(true);
    const res = await getKanbanColumnCardsAction("mentoring", stage.id, filters, pageNum);
