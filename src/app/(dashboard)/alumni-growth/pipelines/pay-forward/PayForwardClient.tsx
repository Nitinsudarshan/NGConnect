...
import { HeartHandshake, ChevronRight, ChevronDown, Filter, ChevronsRightLeft, ChevronsLeftRight } from "lucide-react";
...
const DEFAULT_STAGES = [
  { id: "default-not_eligible", code: "not_eligible", label: "Not eligible (below salary threshold)", sort_order: 0, is_terminal: false },
  { id: "default-eligible_not_contacted", code: "eligible_not_contacted", label: "Eligible, not yet contacted", sort_order: 1, is_terminal: false },
  { id: "default-communicated", code: "communicated", label: "Communicated — discussion in progress", sort_order: 2, is_terminal: false },
  { id: "default-committed_not_started", code: "committed_not_started", label: "Committed — first payment pending", sort_order: 3, is_terminal: false },
  { id: "default-paying_monthly", code: "paying_monthly", label: "Paying — regular monthly", sort_order: 4, is_terminal: false },
  { id: "default-paying_irregular", code: "paying_irregular", label: "Paying — irregular / inconsistent", sort_order: 5, is_terminal: false },
  { id: "default-paused_expected_return", code: "paused_expected_return", label: "Paused — expects to resume", sort_order: 6, is_terminal: false },
  { id: "default-not_paying_no_plan", code: "not_paying_no_plan", label: "Not paying — no resume plan", sort_order: 7, is_terminal: false },
  { id: "default-unreachable", code: "unreachable", label: "Lost contact / unreachable", sort_order: 8, is_terminal: false },
  { id: "default-declined_opted_out", code: "declined_opted_out", label: "Declined / opted out", sort_order: 9, is_terminal: true },
  { id: "default-completed", code: "completed", label: "Completed (target contribution reached)", sort_order: 10, is_terminal: true },
  { id: "default-completed_continuing", code: "completed_continuing", label: "Completed — still contributing beyond cap", sort_order: 11, is_terminal: false },
];

function PayForwardColumn({ stage, activeStages, filters, userEmail, collapsed, setCollapsed, pocOptions }: { stage: any, activeStages: any[], filters: any, userEmail: string, collapsed: boolean, setCollapsed: (v: boolean) => void, pocOptions?: {email: string; name: string}[] }) {
  ...
  const fetchCards = async (pageNum: number, overwrite: boolean = false) => {
    setLoading(true);
    const res = await getKanbanColumnCardsAction("pay_forward", stage.id, filters, pageNum);
  ...
  return (
    <div className="bg-muted/40 p-4 rounded-2xl border border-border/60 flex flex-col min-h-[500px] min-w-[280px]">
      ...
      <div className="space-y-3 flex-1 overflow-y-auto pr-1">
        {cards.map((card) => {
          const prog = card.pfProgress || { counted_toward_cap: 0, cap_inr: 120000 };
...
export default function PayForwardClient({
  ...
  const isStagePassive = (stageCode: string, isTerminal: boolean) => {
    return isTerminal || ["completed", "declined_opted_out", "not_eligible", "unreachable"].includes(stageCode);
  };
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto w-full pb-20">
      ...
      <div className="bg-card border border-border/80 rounded-2xl p-1.5 px-3 shadow-2xs flex flex-row gap-2 items-center">
        ...
        <Select value={campusFilter} onValueChange={setCampusFilter}>
          <SelectTrigger className="h-8 w-[160px] text-xs bg-muted bg-muted/60 hover:bg-muted/80 rounded-lg">
        ...
        <Select value={yearFilter} onValueChange={setYearFilter}>
          <SelectTrigger className="h-8 w-[140px] text-xs bg-muted bg-muted/60 hover:bg-muted/80 rounded-lg">
        ...
        <Select value={supporterFilter} onValueChange={setSupporterFilter}>
          <SelectTrigger className="h-8 w-[180px] text-xs bg-muted bg-muted/60 hover:bg-muted/80 rounded-lg">
