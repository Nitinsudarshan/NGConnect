...
| B5 | `GET /api/integrations/google/auth` + `/callback` | Any signed-in user can start the Google Meet OAuth flow and write the workspace-wide integration token | `learning_center.settings.integrations` edit | |
...
|---|---|---|---|---|
| C1 | `updateAlumniProfileFieldsAction` | Writes fields on any alumni record | `crm.alumni_profile` edit | |
| C2 | `updatePipelineMembershipAction` | Adds/removes an alumnus from a pipeline | `crm.pipelines.*` edit (per pipeline) | |
| C3 | `recordContributionAction` | Records a contribution against an alumnus | `crm.workspace` edit | |
| C4 | `completeFollowupAction` | Closes a follow-up | `crm.follow_ups` edit | |
| C5 | `transferPocAction` | Reassigns lead ownership | `crm.workspace` edit (it already enforces "current owner or Admin") | |
| C6 | `assignToMeAction` | Claims an unassigned alumnus | `crm.workspace` edit | |
| C7 | `getCallReasonsAction`, `getPipelineEligibleStaffAction` | Read config lists | `crm.workspace` view | |
...
| C10 | `fetchExternalTextAction` | Server-side fetch of an arbitrary URL supplied by the caller (usable as a request proxy) | `learning_center.content_hub` edit | |
